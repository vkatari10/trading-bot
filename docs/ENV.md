...
**Note:**  If changes are made to the `.env` file you must `source` or run `. .env` at the root to update these values.
...
Stars (*) next to a setting indicate settings that will later be incporated into JSON config files.